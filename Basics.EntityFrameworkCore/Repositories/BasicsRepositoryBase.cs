...
namespace Basics.EntityFrameworkCore.Repositories
{
    ...
    public abstract class BasicsRepositoryBase<TEntity, TPrimaryKey> : IRepository<TEntity, TPrimaryKey> where TEntity : Entity<TPrimaryKey>
    {
        //定义数据访问上下文对象
        protected readonly BasicsDBContext _dbContext;

        /// <summary>
        /// 通过构造函数注入得到数据上下文对象实例
        /// </summary>
        /// <param name="dbContext"></param>
        public BasicsRepositoryBase(BasicsDBContext dbContext)
        {
    ...
    /// </summary>
    /// <typeparam name="TEntity">实体类型</typeparam>
    public abstract class BasicsRepositoryBase<TEntity> : BasicsRepositoryBase<TEntity, string> where TEntity : Entity
    {
        public BasicsRepositoryBase(BasicsDBContext dbContext) : base(dbContext)
        {
        }
    }
}
